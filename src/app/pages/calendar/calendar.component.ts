...
export class CalendarComponent implements OnInit {
  ...
  async loadRestaurants(): Promise<void> {
    try {
      ...
      console.error('Errore caricamento ristoranti:', e);
    }
  }
  ...
  generateCalendar(): void {
    ...
    const daysBeforeFirstMonday = dayOfWeek === 0 ? 6 : dayOfWeek - 1;

    // Previous month filler days
    const prevMonthDays = Array.from({ length: daysBeforeFirstMonday }, (_, i) => {
      const date = new Date(getYear(this.currentDate), getMonth(this.currentDate), 0);
      date.setDate(date.getDate() - (daysBeforeFirstMonday - 1 - i));
      return { date };
    });
    ...
    const totalCells = Math.ceil((daysBeforeFirstMonday + totalDaysInMonth) / 7) * 7;
    const nextMonthDaysCount = totalCells - (daysBeforeFirstMonday + totalDaysInMonth);
  ...
  getDayTextColor(date: Date): string {
    const bg = this.getDayColor(date);
    if (bg === 'transparent' || bg.toLowerCase() === '#ffffff') return '#333';
  ...
  async saveOrder(): Promise<void> {
    if (!this.orderForm.restaurantId || !this.orderForm.data || !this.orderForm.importo) return;
    ...
    try {
      ...
      console.error('Errore nel salvataggio ordine:', e);
    }
  }
}